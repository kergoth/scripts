Default selection includes changed targets and managed modifiers only.
  ...
  $ bash "$TESTDIR/../../chezmoi-diff-managed" | awk 'NF'
...
Directory arguments use scoped managed discovery.
  ...
  $ bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/config" | awk 'NF'
...
Comparator errors continue later comparisons and preserve a failing exit status.
  ...
  > EOF
  $ chmod +x bin/difft
  $ (set -o pipefail; bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/changed.txt" "$PWD/destination/modified.json" | awk 'NF')
  == managed (rendered): */source/changed.txt.tmpl == (glob)
  == destination: */destination/changed.txt == (glob)
  src.txt changed.txt
  == managed (rendered): */source/modify_modified.json == (glob)
...
The plain diff fallback continues after expected differences.
  ...
  $ PATH="$PWD/bin:/usr/bin:/bin" bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/changed.txt" "$PWD/destination/modified.json" | awk 'NF'
  == managed (rendered): */source/changed.txt.tmpl == (glob)
  == destination: */destination/changed.txt == (glob)
  src.txt changed.txt
  == managed (rendered): */source/modify_modified.json == (glob)
...
JSON with comments falls back to a syntax-aware text diff.
  ...
  $ bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/zed-settings.json" | awk 'NF'
...
Structured differences do not stop later comparisons.
  ...
  $ bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/modified.json" "$PWD/destination/regular.json" | awk 'NF'
  == managed (rendered): */source/modify_modified.json == (glob)
  == destination: */destination/modified.json == (glob)
  src.json modified.json
...
Identical rendered baselines do not invoke semantic diffing.
  ...
  $ bash "$TESTDIR/../../chezmoi-diff-managed" "$PWD/destination/identical.txt"
